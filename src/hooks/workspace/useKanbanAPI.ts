...
// 컬럼 ID를 상태로 변환
const getStatusFromColumnId = (columnId: number, columns: ColumnResponse[]): TaskStatus => {
...
// 상태를 컬럼 ID로 변환
const getColumnIdFromStatus = (status: TaskStatus, columns: ColumnResponse[]): number => {
...
export function useKanbanAPI(kanbanId?: number) {
    ...
    try {
      ...
      if (tasksResponse.success && columnsResponse.success) {
        ...
        tasksResponse.data.forEach(task => {
          const kanbanCard = mapTaskToKanbanCard(task, columnsResponse.data);
          groupedTasks[kanbanCard.status].push(kanbanCard);
        });

        setTasks(groupedTasks);
      } else {
        setError(tasksResponse.error || columnsResponse.error || "데이터 로드 실패");
      }
    } catch (err) {
  ...
  // 태스크 상세 정보 가져오기
  const getTaskDetailById = useCallback(async (taskId: string): Promise<TaskDetailResponse | null> => {
  ...
  // 태스크 생성
  const handleTaskCreate = useCallback(async (task: KanbanCardProps): Promise<boolean> => {
    if (!kanbanId) {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "태스크 생성 중 오류가 발생했습니다.";
  ...
  // 태스크 업데이트
  const handleTaskUpdate = useCallback(async (task: KanbanCardProps): Promise<boolean> => {
    if (!kanbanId) return false;
    ...
    try {
      ...
      if (response.success) {
        ...
        setTasks(prev => {
          const newTasks = { ...prev };
          Object.keys(newTasks).forEach(status => {
            const taskIndex = newTasks[status].findIndex(t => t.id === task.id);
            if (taskIndex !== -1) {
